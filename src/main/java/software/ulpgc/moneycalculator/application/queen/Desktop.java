...
public class Desktop extends JFrame {
    ...
    private JTextArea outputOperations;
    ...
    private JPanel panel() {
        ...
        panel.add(operationsOutput());
        return panel;
    }

    private Component showHistoryButton() {
        JButton showHistoryButton = new JButton("Show History");
        showHistoryButton.addActionListener(e -> {
            outputOperations.setText("");
            commands.get("history").execute();
        });
    ...
    private JScrollPane operationsOutput() {
        JTextArea textArea = textArea();
        outputOperations = textArea;
    ...
    public OperationDisplay operationDisplay() {
        return operation -> outputOperations.append(format(operation));
    }

    private String format(ExchangeOperation operation) {
        StringBuilder sb = new StringBuilder();

        sb.append(operation.date().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
                    .append(" -> result: ")
                    .append(operation.result().amount())
                    .append(" ")
                    .append(operation.result().currency().code())
                    .append(" from: ")
                    .append(operation.exchange().from())
                    .append("\n");
